...
import com.example.niki.fieldoutlookandroid.helper.authentication_service.AuthenticateAsyncTask;
 ...
 */
public class LoginActivity extends Activity implements LoaderCallbacks<Cursor>, AuthenticateUserReciever.Listener  {
     ...
     */
    private AuthenticateAsyncTask mAuthTask = null;
    ...
    public void attemptLogin() {
        ...
        if (cancel) {
           ...
           // showProgress(true);
            mAuthTask=new AuthenticateAsyncTask(new AuthenticateAsyncTask.AuthenticationDelegate() {
                @Override
                public void processFinish(AuthenticationResponse response) {
                    Global.GetInstance().setUser(response);
                    showProgress(false);
                    mAuthTask=null;
                    if(response!=null && response.GetIsAuthenticated()!=null && response.GetIsAuthenticated().toLowerCase().equals("true")) {
            ...
            showProgress(true);
           // startService(createCallingIntent(email, password));
    ...
    public Loader<Cursor> onCreateLoader(int i, Bundle bundle) {
        return new CursorLoader(this,
                ...
                // Show primary e
                // l address if the user hasn't specified one.
    ...
    private Intent createCallingIntent(String mEmail, String mPassword) {
        Intent i = new Intent(this, IntentServiceHelper.class);
        AuthenticationResponse authenticationResponse= new AuthenticationResponse();
        authenticationResponse.SetPassword(mPassword);
        authenticationResponse.SetUsername(mEmail);
        //Looper.prepare();
        //Looper.prepare();
        AuthenticateUserReciever receiver = new AuthenticateUserReciever(new Handler());
        receiver.setListener(this);
        i.putExtra("rec", receiver);
        i.putExtra("authenticateResponse", authenticationResponse);
        return i;
    }
    // Callback method
    @Override
